...
class ApiService {
  ...
  static Future<Colaborador> createColaborador(Colaborador colaborador) async {
    try {
      final response = await _dio.post('/colaboradores', data: colaborador.toJson());
  ...
  // --- Documentos ---
  static Future<List<Documento>> getDocumentosPorColaborador(int colaboradorId) async {
    try {
      final response = await _dio.get('/documentos', queryParameters: {
  ...
  // --- Vínculos ---
  static Future<List<Vinculo>> getVinculosPorColaborador(int colaboradorId) async {
    try {
      final response = await _dio.get('/vinculos', queryParameters: {
        'colaborador_id': colaboradorId,
      });
      return (response.data as List)
